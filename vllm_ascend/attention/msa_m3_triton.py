...
# Prefill score-prepare launch policy. Mirrors _prefill_score_finalize_kernel:
# the query axis is tiled (BLOCK_SIZE_Q) and a tile size is chosen from
# PREP_QUERY_TILE_CANDIDATES so the total program count is near
# PREP_TARGET_PROGRAM_COUNT. Each program owns a [BLOCK_SIZE_Q] query tile for
# one (batch, head) and streams over the block range in BLOCK_SIZE_K tiles.
...
def _choose_prep_query_tile_size(
    ...
    """Pick the query tile for prefill score-prepare (target ~64 programs)."""
...
# Prefill top-k invalid-index-mask launch policy. Mirrors
# _prefill_topk_invalid_index_mask_kernel: query axis is tiled (BLOCK_SIZE_Q)
# with the tile chosen to target ~64 programs while bounding the 2-D
# [BLOCK_SIZE_Q, topk] tile to INVALID_MASK_MAX_TILE_ELEMENTS.
...
def _prefill_index_score_prepare_for_topk_kernel(
    ...
    # Grid: (cdiv(max_query_len, BLOCK_SIZE_Q), batch * index_head_count). The
    # query axis is tiled (not folded inside), so query parallelism is preserved;
    # BLOCK_SIZE_Q is chosen at the launch site to target ~64 total programs
    # (mirrors _prefill_score_finalize_kernel). Each program owns one
    # [BLOCK_SIZE_Q] query tile for one (batch, head) and vectorizes the
    # tail/init/local masking over [BLOCK_SIZE_Q, BLOCK_SIZE_K] score tiles.
...
def _topk_index_mask_invalid_prefill_kernel(
    ...
    # Grid: (cdiv(max_query_len, BLOCK_SIZE_Q), batch * index_head_count). The
    # query axis is tiled (not folded inside); BLOCK_SIZE_Q is chosen at the
    # launch site to target ~64 total programs while bounding the 2-D
    # [BLOCK_SIZE_Q, topk] tile (mirrors _prefill_topk_invalid_index_mask_kernel).
    # Each program vectorizes the invalid-index masking over one query tile.
    query_tile_id = tl.program_id(0)
    batch_head_id = tl.program_id(1)
    batch_id = batch_head_id // index_head_count
    head_id = batch_head_id % index_head_count
...
def minimax_m3_index_topk(
    ...
    ``_prefill_index_score_prepare_for_topk_kernel`` is always run before
    ``torch.topk`` to pad the score tail with ``-inf`` and force the init/local
    blocks (mirroring the reference path).
    """
    num_idx_heads = score.shape[0]
    batch = cu_seqlens_q.shape[0] - 1
    total_q = score.shape[1]
    max_block = score.shape[2]
    # Tiled-query grid mirroring _prefill_score_finalize_kernel: pick a query
    # tile so the total program count is near PREP_TARGET_PROGRAM_COUNT. All
    # factors are shape-constant, so the grid is stable for cudagraph.
